...
#else
// rom area for roms (16 Megabyte)
unsigned char __attribute__ ((__aligned__(4))) mainrom[8*1024*1024];
#endif
...
const unsigned char ngpmainram[256] = {
	// 0x00
...
void mem_init()
{
	...
	fastmemset(&mainram,0,sizeof(mainram));
    switch(m_emuInfo.machine) {
	case NGP:
	case NGPC:
		z80ram = &mainram[0x7000];
		if(!loadBIOS())
		{
		    realBIOSloaded = 0;
            fastmemset(&cpurom,0,sizeof(cpurom));
            ...
            *((unsigned long *)(&cpurom[0xff04])) = (unsigned long)0x00fff000;
            ...
            for(i=0; i<sizeof(ngpmainram); i++)
            {
                mainram[i] = ngpmainram[i];
            }
            // setup interrupt vectors in RAM
            for(i=0; i<18; i++) {
                *((unsigned long *)(&mainram[0x6FB8+4*i])) = (unsigned long)0x00FFF800;
            }
        }
		else
		{
            ...
            mainram[0x006F] = 0x4E;  //Watchdog timer
            mainram[0x00B8] = 0xAA;  //No z80 at boot
            mainram[0x00B9] = 0xAA;
            mainram[0x6F80] = 0xFF; //Lots of battery power!
            mainram[0x6F81] = 0x03;
            mainram[0x6F95] = 0x10;
            mainram[0x6F91] = 0x10; // Colour bios
            mainram[0x6F84] = 0x40; // "Power On" startup
            mainram[0x6F85] = 0x00; // No shutdown request
            mainram[0x6F86] = 0x00; // No user answer (?)

            mainram[0x6F87] = 0x01; //English
		}

        mainram[0x8000] = 0xC0;		// Enable generation of VBlanks by default
        mainram[0x8004] = 0xFF;	mainram[0x4005] = 0xFF;
        mainram[0x8006] = 0xC6;
        for(i=0; i<5; i++) {
            mainram[0x8101+4*i] = 0x07;	mainram[0x8102+4*i] = 0x07;	mainram[0x8103+4*i] = 0x07;
        }
        mainram[0x8118] = 0x07;
        mainram[0x83E0] = mainram[0x83F0] = 0xFF; mainram[0x83E1] = mainram[0x83F1] = 0x0F;
        mainram[0x83E2] = mainram[0x83F2] = 0xDD; mainram[0x83E3] = mainram[0x83F3] = 0x0D;
        mainram[0x83E4] = mainram[0x83F4] = 0xBB; mainram[0x83E5] = mainram[0x83F5] = 0x0B;
        mainram[0x83E6] = mainram[0x83F6] = 0x99; mainram[0x83E7] = mainram[0x83F7] = 0x09;
        mainram[0x83E8] = mainram[0x83F8] = 0x77; mainram[0x83E9] = mainram[0x83F9] = 0x07;
        mainram[0x83EA] = mainram[0x83FA] = 0x44; mainram[0x83EB] = mainram[0x83FB] = 0x04;
        mainram[0x83EC] = mainram[0x83FC] = 0x33; mainram[0x83ED] = mainram[0x83FD] = 0x03;
        mainram[0x83EE] = mainram[0x83FE] = 0x00; mainram[0x83EF] = mainram[0x83FF] = 0x00;
