...
void sound_update(_u16* chip_buffer, int length_bytes)
{
	...
	while (length_bytes)
	{
		...
		*(chip_buffer++) = (sample_chip_tone() + sample_chip_noise()) >> 1;
		...
		length_bytes--;
		  SDL_DC_SetSoundBuffer( chip_buffer);
	}
}
...
void dac_writeL(unsigned char data)
{
	...
	//Write to buffer
#ifdef __GP32__
	dacBufferL[dacLBufferWrite++] = data<<8;
#else
...
void sound_init(int SampleRate)
{
	...
	memset1(&toneChip, 0, sizeof(SoundChip));

	//Initialise Right Chip
	memset1(&noiseChip, 0, sizeof(SoundChip));
...
int sound_system_init()
{
    ...
    fmt.freq = CHIP_FREQUENCY;  //11025 is good for dac_ sound
