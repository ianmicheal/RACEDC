...
inline void tlcsMemWriteBaddrB(unsigned char addr, unsigned char data)
{
    ...
    case 0xA0:	// L CH Sound Source Control Register
        if (mainram[0xB8] == 0x55 && mainram[0xB9] == 0xAA)
...
inline void tlcsMemWriteL(unsigned long addr, unsigned long data)
{
    tlcsMemWriteB(addr,   data);
    tlcsMemWriteB(addr+1, data>>8);
    tlcsMemWriteB(addr+2, data>>16);
    tlcsMemWriteB(addr+3, data>>24);
}
...
unsigned char mirrTable[256] = {  //Flavor speed hack
...
int daar()  // DAA r    11001rrr 00010000
{
    ...
    gen_regsSR = gen_regsSR & ~(SF|ZF|HF|CF);
    if (gen_regsSR & NF)
...
unsigned short myDivB(unsigned short i, unsigned char j)
{
    if (!j)
    {
        gen_regsSR|= VF;
...
unsigned int myDivW(unsigned int i, unsigned short j)
{
    if (!j)
    {
        gen_regsSR|= VF;
...
#define LDCF(A,B) if (A & (1<<(B))) gen_regsSR|= CF; \
    else    gen_regsSR&= ~CF;
...
{
    if (gen_regsSR & CF)
        mem_writeB(mem,mem_readB(mem) | (1<<(lastbyte&7)));
    else
        mem_writeB(mem,mem_readB(mem) & ~(1<<(lastbyte&7)));
    return 8;
}
...
{
    if (gen_regsSR & CF)
        mem_writeB(mem,mem_readB(mem) | (1<<*cregsB[1]));
    else
        mem_writeB(mem,mem_readB(mem) & ~(1<<*cregsB[1]));
    return 8;
}
...
#define ANDCF(A,B) if ((A & (1<<(B))) && (gen_regsSR & CF)) gen_regsSR|= CF; \
    else          gen_regsSR&= ~CF;
...
#define BIT(A,B) gen_regsSR = (gen_regsSR & ~(ZF|NF)) | ((A & (1<<(B))) ? HF : HF|ZF);
...
inline int VECT_INTLVSET(unsigned char interrupt, unsigned char level)
{
    ...
    {
        case 0x00:
        mainram[0x70] = (mainram[0x70] & 0xf0) |  (level & 0x07);
        break;
        case 0x01:
        mainram[0x71] = (mainram[0x71] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x02:
        mainram[0x73] = (mainram[0x73] & 0xf0) |  (level & 0x07);
        break;
        case 0x03:
        mainram[0x73] = (mainram[0x73] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x04:
        mainram[0x74] = (mainram[0x74] & 0xf0) |  (level & 0x07);
        break;
        case 0x05:
        mainram[0x74] = (mainram[0x74] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x06:
        mainram[0x79] = (mainram[0x79] & 0xf0) |  (level & 0x07);
        break;
        case 0x07:
        mainram[0x79] = (mainram[0x79] & 0x0f) | ((level & 0x07)<<4);
        break;
        case 0x08:
        mainram[0x7a] = (mainram[0x7a] & 0xf0) |  (level & 0x07);
        break;
        case 0x09:
        mainram[0x7a] = (mainram[0x7a] & 0x0f) | ((level & 0x07)<<4);
        break;
    }
    return 100;
}
...
inline void tlcs_interrupt(int irq)
{
    ...
    int i;
    int level = ((irq&1) ? ((mainram[0x70+(irq>>1)])>>4)&7 : (mainram[0x70+(irq>>1)])&7 );
    if (level==7)
        level = 0;
...
#define DMA0V mainram[0x7C]
#define DMA1V mainram[0x7D]
#define DMA2V mainram[0x7E]
#define DMA3V mainram[0x7F]
...
#define T8RUN mainram[0x20]
#define TREG0 mainram[0x22]
#define TREG1 mainram[0x23]
#define T01MOD mainram[0x24]
#define TREG2 mainram[0x26]
#define TREG3 mainram[0x27]
#define T23MOD mainram[0x28]
int    timer0 = 0;
int    timer1 = 0;
int    timer2 = 0;
int    timer3 = 0;
...
#define TIMER_BASE_RATE  (170)  //what NeoPop seemed to use
...
inline void tlcsTimers(int stateChange)
{
    ...
    // // check for updates
    // if (0) {
    //  TREG0 = mainram[0x22] ? mainram[0x22] : 256;
    //  TREG1 = mainram[0x23] ? mainram[0x23] : 256;
    //  TREG2 = mainram[0x26] ? mainram[0x26] : 256;
    //  TREG3 = mainram[0x27] ? mainram[0x27] : 256;
    // }
    // timer 0 + 1
...
#define FIXED_FRAMESKIP 0x1
#define MAX_SKIPFRAMES 0
#endif
