...
png_to_texture("/cd/image/back.png", back_tex, PNG_NO_ALPHA);
}
/* draw background */ 
void draw_back(void) 
{
    ...
    pvr_poly_cxt_txr(&cxt, PVR_LIST_OP_POLY, PVR_TXRFMT_RGB565, 512, 512, back_tex,PVR_FILTER_TRILINEAR2); 
...
//	for (i=0; i<14 && (top+i)<num_entries; i++) {
	for (i=0; i<10 && (top+i)<num_entries; i++) {
		int li;
		draw_poly_strf(32.0f, y, 150.0f, 1.0f, 1.0f, 1.0f, 1.0f,
			"%s", entries[top+i].fn);
...
//			printf("%s =? %s \r\n",workstring,lst_entries[li].fn);
 ...
 sndoggvorbis_start("/cd/music/theme.ogg",0);
...
void check_controller() {
	...
	if (1)//(!(sndoggvorbis_isplaying())) && (lst_playing != -1))
	{
		lst_playing++;
...
//	draw_wave(); 
		...
		draw_poly_strf(5.0f, 20.0f, 100.0f, 1.0f, 1.0f, 1.0f, 1.0f,
			"RACEDC ported by Troy Davis(GPF)(U) Ian Micheal R4");
